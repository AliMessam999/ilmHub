...
@section('header', 'Add New Lecture')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div class="col-span-2">
                ...
                <textarea name="description" id="description" rows="4" 
                    class="w-full border border-gray-300 rounded-md shadow-sm px-4 py-2 focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="language" id="language" value="{{ old('language', 'English') }}" required placeholder="e.g., English, Urdu, Arabic"
                    ...
                    @foreach($topics as $topic)
                        <label class="flex items-center text-sm text-gray-700 cursor-pointer">
                            <input type="checkbox" name="topics[]" value="{{ $topic->id }}" 
                                ...
                                {{ in_array($topic->id, (array)old('topics', [])) ? 'checked' : '' }}>
