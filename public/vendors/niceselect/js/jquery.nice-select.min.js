...
(function($) {
    $.fn.niceSelect = function(method) {
        ...
        $(document).on("keyup.nice-select-search", ".nice-select", function () {
            $(".nice_Select2.open .list").css("margin-top","52px")
            var $self = $(this);
            var $text = $self.find(".nice-select-search").val().toLowerCase(); // Get the search text in lowercase
            var $options = $self.find("ul li"); // Get all the options
            
            if ($text === "") {
                $options.show(); // Show all options if the search text is empty
            } else if ($self.hasClass("open")) {
                $options.each(function () {
                    var $this = $(this);
                    var $optionText = $this.text().toLowerCase(); // Get the option text in lowercase
                    
                    // Match option text with search text
                    if ($optionText.indexOf($text) !== -1) {
                        $this.show(); // Show options that match the search text
                    } else {
                        $this.hide(); // Hide options that don't match the search text
                    }
                });
            }
        
            // Highlight the first visible option as focus
            $self.find(".option"),
                $self.find(".focus").removeClass("focus"),
                $self.find(".selected").removeClass("focus"),
                $self.find("ul li:visible").first().addClass("focus"); // Set focus to the first visible option
        });
